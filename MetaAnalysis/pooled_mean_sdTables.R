...
###### Re-run the code for the sensitivity analysis
rm(list=ls())
####### packages
library(dplyr)
library(tidyr)
...
continuous_data <- function(site_df, rdata_fileName, files_path){
  ...
  for (i in 1:nrow(site_df)){
    ...
    # load the file
    print(i)
    filename = paste(files_path, site_df$site[i], "/original/",site_df$site[i],"_", rdata_fileName,sep="")
    load(filename)
...
age_hospitalization_formatted %>%
  flextable::flextable() %>%
  flextable::save_as_docx(path = "/Users/alba/Desktop/age_lengthHosp_summary_original.docx")
...
labsAll_results_formatted %>%
  flextable::flextable() %>%
  flextable::save_as_docx(path = "/Users/alba/Desktop/labs_summary_original.docx")
