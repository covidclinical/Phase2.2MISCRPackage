...
misc_overview <- function( integrated_df, output_plot = FALSE, output_df = TRUE, cbPalette = cbPalette, verbose ){
  ...
  if( output_df == TRUE ){
    ...
    write.table( x         = misc_icu,
                 file      = file.path(dir.output,paste0(currSiteId, "_MISCpatientICU.txt")),
                 col.names = TRUE,
                 row.names = FALSE,
                 quote     = FALSE,
                 sep       = "\t" )
    write.table( x         = hospitalization_length,
                 file      = file.path(dir.output,paste0(currSiteId, "_MISChospitalization_length.txt")),
                 col.names = TRUE,
                 row.names = FALSE,
                 quote     = FALSE,
                 sep       = "\t" )
  ...
  if( output_plot == TRUE ){
    ...
    misc_icu_plot <- ggplot(misc_icu, aes(fill=variant_misc, y=icu_patients, x=var)) +
      ...
      xlab("Number of MISC patients") + ylab("Variant") +
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5))
    ggplot2::ggsave(filename=file.path(dir.output,paste0(currSiteId, "_misc_icu_plot.png")),plot=print(misc_icu_plot))
    ...
    misc_hosp_length_dsitribution_plot <- first_hospitalization %>%
      ...
      xlab("Days hospitalized") + ylab("Variant") +
      theme_bw() +
      theme(plot.title = element_text(hjust = 0.5))
    ggplot2::ggsave(filename=file.path(dir.output,paste0(currSiteId, "_misc_hosp_length_dsitribution_plot.png")),plot=print(misc_hosp_length_dsitribution_plot))
  }
}
